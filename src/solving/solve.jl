...
function endgame(solvers, start_solutions, endgame_zone_results)
    ...
    if t₀ == t_endgame
        return Parallel.tmap(solvers, 1:n) do solver, tid, k
            ...
            PathResult(solver.prob, x₁, t₀, r, solver.cache.pathresult, solver.patchswitcher)
        end
    end
...
@inline function runendgame(solver, tid, k, start_solutions, endgame_zone_results, t_endgame, t₀)
    ...
    if r.returncode == :success
        ...
        return PathResult(solver.prob, x₁, t₀, result, solver.cache.pathresult, solver.patchswitcher)
    else
        return PathResult(solver.prob, x₁, t₀, r, solver.cache.pathresult, solver.patchswitcher)
    end
end
